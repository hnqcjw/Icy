...
void executeLine(const std::string& line, size_t& currentLine) {
    ...
    if (trimmed.find("printLine(") != std::string::npos) {
    ...
    if (trimmed.find("print ") != std::string::npos) {
        size_t start = trimmed.find(' ');
        size_t end = trimmed.rfind(';');
        if (start != std::string::npos && end != std::string::npos && end > start) {
            std::string arg = trimmed.substr(start + 1, end - start - 1);
    ...
    if (trimmed.find("goToLine ") != std::string::npos) {
        size_t start = trimmed.find(' ');
        size_t end = trimmed.rfind(';');
        std::string arg = trimmed.substr(start + 1, end - start - 1);
        if (start != std::string::npos && end != std::string::npos && end > start) {
...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(sourceFile, line)) {
        ...
        if (!inFunction) {
            if (trimmed.find("when_i_call ") == 0) { // Check if it starts with "func "
                size_t nameStart = 12; // Start after "func "
